...
import numpy as np
from sgm.modules.encoders.siglip_encoder import SiglipVisionTower
from sgm.modules.encoders.dinov2_encoder import DinoV2VisionTower
...
class SATVideoDiffusionEngine_withact(nn.Module):
    def __init__(self, args, **kwargs):
        super().__init__()
        ...
        lora_train = model_config.get("lora_train", False)
        self.use_pd = model_config.get("use_pd", False)  # progressive distillation
        self.use_latent_action = model_config.get("use_latent_action", False)
        self.latent_action_dim = model_config.get("latent_action_dim", 50)  # progressive distillation
        ...
        self.noised_image_all_concat = model_config.get("noised_image_all_concat", False)
        self.noised_image_dropout = model_config.get("noised_image_dropout", 0.0)
        self.use_action_firstframe_concat = model_config.get("use_action_firstframe_concat", False) 
        if args.fp16:
            dtype = torch.float16
            dtype_str = "fp16"
        elif args.bf16:
    ...
    def forward(self, x, batch):
        ...
        # loss_mean = loss.mean()
        # loss_dict = {"loss": loss_mean}
        loss, loss_dict = self.loss_fn(self.model, self.denoiser, self.conditioner, x, batch)
        loss_mean = loss.mean()
        loss_dict["total_loss"] = loss_mean
        return loss_mean, loss_dict
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        if self.noised_image_input:
            ...
            if random.random() < self.noised_image_dropout:
                image = torch.zeros_like(image)
            batch["concat_images"] = image
            if self.use_action_firstframe_concat:
                batch["action_concat"] = image[:,0]
        # print("SATVideoDiffusionEngine embeding", time.time()-curr_time)
        curr_time = time.time()

        gc.collect()
        torch.cuda.empty_cache()        
        # print("SATVideoDiffusionEngine collect data", time.time()-curr_time)
        curr_time = time.time()
    
        loss, loss_dict = self(x, batch)

        # print("SATVideoDiffusionEngine all forward and loss", time.time()-curr_time)
        # curr_time = time.time()
    ...
    def sample(
        ...
        **kwargs,
        ...
        action_input = kwargs["batch"]["actions"]
        action_randn = torch.randn_like(action_input).to(torch.float32).to(self.device)
        if hasattr(self, "seeded_noise"):
            action_randn = self.seeded_noise(action_randn)

        if prefix is not None:
            randn = torch.cat([prefix, randn[:, prefix.shape[1] :]], dim=1)
        if action_prefix is not None:
            action_randn = torch.cat([action_prefix, action_randn[:, action_prefix.shape[1] :]], dim=1)
        ...
        if mp_size > 1:
            ...
            src = global_rank * mp_size
            torch.distributed.broadcast(randn, src=src, group=mpu.get_model_parallel_group())
            torch.distributed.broadcast(action_randn, src=src, group=mpu.get_model_parallel_group())
        ...
        _, action_samples = self.sampler(denoiser, randn, action_randn, cond, uc=uc, scale=scale, scale_emb=scale_emb, auxinfo_dict=auxinfo_dict)
        action_samples = action_samples .to(self.dtype)
    ...
    def log_video(
        ...
        **kwargs,
        ...
        if self.noised_image_input:
            ...
            c["concat"] = image
            uc["concat"] = image
            samples, action_samples = self.sample(c, shape=z.shape[1:], uc=uc, batch_size=N, **sampling_kwargs)  # b t c h w
            samples = samples.permute(0, 2, 1, 3, 4).contiguous()

            log["eval_action_l1"] = torch.mean(batch["actions"] - action_samples).detach().item()
            ...
            log["eval_action_l1"] = torch.mean(batch["actions"] - action_samples)
