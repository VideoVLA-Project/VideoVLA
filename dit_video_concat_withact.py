...
class ImagePatchEmbeddingMixin_withact(BaseMixin):
    def __init__(
        self,
        in_channels,
        hidden_size,
        ...
        if action_hidden_size is not None:
            self.action_proj = nn.Linear(action_hidden_size, hidden_size)
        else:
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        if self.action_proj is not None:
            action_emb = self.action_proj(kwargs["action_input_withdenoiser"]) # from  sgm/modules/diffusionmodules/denoiser.py DiscreteDenoiser_withact
            emb = torch.cat((emb, action_emb), dim=1)  # (b,n_t+t*n_i,d)
...
class ImagePatchEmbeddingMixin_withact_SigSelfattn(BaseMixin):
    def __init__(
        self,
        in_channels,
        hidden_size,
        ...
        text_hidden_size=None,
        action_hidden_size=None,
        vision_encoder_hidden_size=None,
        # action_concat_input
    ):
        super().__init__()
        self.proj = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
        ...
        if action_hidden_size is not None:
            ...
            self.action_proj = None
        if vision_encoder_hidden_size is not None:
            self.vision_encoder_proj = nn.Linear(vision_encoder_hidden_size, hidden_size)
        else:
            self.vision_encoder_proj = None
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        if self.action_proj is not None:
            action_emb = self.action_proj(kwargs["action_input_withdenoiser"]) # from  sgm/modules/diffusionmodules/denoiser.py DiscreteDenoiser_withact
            emb = torch.cat((emb, action_emb), dim=1)  # (b,n_t+t*n_i,d)
        if self.vision_encoder_proj is not None:
            vision_emb = self.vision_encoder_proj(kwargs["vision_feature"]) # from  sgm/modules/diffusionmodules/denoiser.py DiscreteDenoiser_withact
            emb = torch.cat((emb, vision_emb), dim=1)  # (b,n_t+t*n_i,d)
...
class Rotary3DPositionEmbeddingMixin_withact(BaseMixin):
    def __init__(
        self,
        height,
        width,
        ...
        self.use_robotid_emb = use_robotid_emb

        if learnable_pos_embed:
            num_patches = height * width * compressed_num_frames + text_length
            self.pos_embedding = nn.Parameter(torch.zeros(1, num_patches, int(hidden_size)), requires_grad=True)
            if self.action_length > 0:
                self.action_pos_embedding = nn.Parameter(torch.zeros(1, self.action_length, int(hidden_size)), requires_grad=True)
                if self.use_robotid_emb:
                    self.action_robotid_embedding = nn.Embedding(max_num_robots, embedding_dim=int(hidden_size))
                else:
                    self.action_robotid_embedding = None
            else:
    ...
    def position_embedding_forward(self, position_ids, **kwargs):
        if self.pos_embedding is not None:
            text_video_emb = self.pos_embedding[:, :self.text_length + kwargs["seq_length"]]

            if self.action_pos_embedding is not None:
                action_pos_emb = self.action_pos_embedding[:, :kwargs["act_length"]] # 1, N_act, C
                

                if "robot_ids" in kwargs.keys() and self.use_robotid_emb:
                    # print("use_robotid_emb")
                    action_robot_emb = self.action_robotid_embedding(kwargs["robot_ids"]).unsqueeze(1) # B, 1, C
                    action_pos_emb = action_pos_emb + action_robot_emb # 1, N_act, C    +    B, 1, C -> B, N_act, C 
                
                return torch.cat([text_video_emb.repeat(action_pos_emb.shape[0],1,1), action_pos_emb],dim=1)
    ...
    def attention_fn(
        ...
        **kwargs,
        ...
        attention_fn_default = HOOKS_DEFAULT["attention_fn"]
        # [text_feat, pixels_feats, actions_feats]
        query_layer[:, :, self.text_length : -self.action_length] = self.rotary(query_layer[:, :, self.text_length : -self.action_length])
        key_layer[:, :, self.text_length : -self.action_length] = self.rotary(key_layer[:, :, self.text_length : -self.action_length])
        if self.rot_v:
            value_layer[:, :, self.text_length : -self.action_length] = self.rotary(value_layer[:, :, self.text_length : -self.action_length])
...
class Rotary3DPositionEmbeddingMixin_withact_SigSelfattn(BaseMixin):
    def __init__(
        self,
        height,
        width,
        ...
        self.text_length = text_length
        self.action_length = action_length
        self.vision_embeding_length = vision_embeding_length
        ...
        if learnable_pos_embed:
            ...
            if vision_embeding_length>0:
                self.vision_pos_embedding = nn.Parameter(torch.tensor(get_1d_sincos_pos_embed_from_grid(int(hidden_size), torch.arange(vision_embeding_length))), requires_grad=False)
            else:
                self.vision_pos_embedding = None
    ...
    def position_embedding_forward(self, position_ids, **kwargs):
        if self.pos_embedding is not None:
            ...
            if self.action_pos_embedding is not None:
                ...
                text_video_action_emb = torch.cat([text_video_emb.repeat(action_pos_emb.shape[0],1,1), action_pos_emb],dim=1)

                if self.vision_pos_embedding is not None:
                    vision_pos_emb = self.vision_pos_embedding.unsqueeze(0).repeat(text_video_action_emb.shape[0],1,1)
                    text_video_action_vision_emb = torch.cat([text_video_action_emb, vision_pos_emb],dim=1)
                    return text_video_action_vision_emb

                return text_video_action_emb
    ...
    def attention_fn(
        ...
        **kwargs,
        ...
        attention_fn_default = HOOKS_DEFAULT["attention_fn"]
        # [text_feat, pixels_feats, actions_feats]
        query_layer[:, :, self.text_length : -self.action_length-self.vision_embeding_length] = self.rotary(query_layer[:, :, self.text_length : -self.action_length-self.vision_embeding_length])
        key_layer[:, :, self.text_length : -self.action_length-self.vision_embeding_length] = self.rotary(key_layer[:, :, self.text_length : -self.action_length-self.vision_embeding_length])
        if self.rot_v:
            value_layer[:, :, self.text_length : -self.action_length-self.vision_embeding_length] = self.rotary(value_layer[:, :, self.text_length : -self.action_length-self.vision_embeding_length])
...
class Rotary3DPositionEmbeddingMixin_varysize(BaseMixin):
    def __init__(
        self,
        height,
        width,
...
class FinalLayerMixin_withact(BaseMixin):
    def __init__(
        self,
        hidden_size,
        time_embed_dim,
        ...
        self.action_norm_final = nn.LayerNorm(hidden_size, elementwise_affine=elementwise_affine, eps=1e-6)
        self.action_linear = nn.Linear(hidden_size, output_action_dim, bias=True)
        self.action_adaLN_modulation = nn.Sequential(nn.SiLU(), nn.Linear(time_embed_dim, 2 * hidden_size, bias=True))
    ...
    def final_forward(self, logits, **kwargs):
        ...
        x, emb = logits[:, kwargs["text_length"] : -kwargs["act_length"], :], kwargs["emb"]  # x:(b,(t n),d)
        shift, scale = self.adaLN_modulation(emb).chunk(2, dim=1)
        x = modulate(self.norm_final(x), shift, scale)
        x = self.linear(x)
        video_pred_noise = unpatchify(
            x,
            c=self.out_channels,
            p=self.patch_size,
        ...
        action_x, emb = logits[:, -kwargs["act_length"]:, :], kwargs["emb"]  # x:(b,(t n),d)
        action_shift, action_scale = self.action_adaLN_modulation(emb).chunk(2, dim=1)
        action_x = modulate(self.action_norm_final(action_x), action_shift, action_scale)
        action_pred_noise = self.action_linear(action_x)
        return video_pred_noise, action_pred_noise 
        # return 

    def reinit(self, parent_model=None):
        nn.init.xavier_uniform_(self.linear.weight)
        nn.init.constant_(self.linear.bias, 0)
        nn.init.xavier_uniform_(self.action_linear.weight)
        nn.init.constant_(self.action_linear.bias, 0)




class FinalLayerMixin_withact_SigSelfattn(BaseMixin):
    def __init__(
        self,
        hidden_size,
        time_embed_dim,
    ...
    def final_forward(self, logits, **kwargs):
        ...
        x, emb = logits[:, kwargs["text_length"] : -(kwargs["act_length"]+self.vision_embeding_length), :], kwargs["emb"]  # x:(b,(t n),d)
        shift, scale = self.adaLN_modulation(emb).chunk(2, dim=1)
        x = modulate(self.norm_final(x), shift, scale)
        x = self.linear(x)
        video_pred_noise = unpatchify(
        ...
        action_x, emb = logits[:, -(kwargs["act_length"]+self.vision_embeding_length):-self.vision_embeding_length, :], kwargs["emb"]  # x:(b,(t n),d)
        action_shift, action_scale = self.action_adaLN_modulation(emb).chunk(2, dim=1)
        action_x = modulate(self.action_norm_final(action_x), action_shift, action_scale)
        action_pred_noise = self.action_linear(action_x)
        return video_pred_noise, action_pred_noise 
...
class DiffusionTransformer_withact(BaseModel):
    def __init__(
        self,
        transformer_args,
        num_frames,
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        # print(f"DiffusionTransformer curr_time",time.time())
        curr_time = time.time()
        b, t, d, h, w = x.shape
        if x.dtype != self.dtype:
            x = x.to(self.dtype)
        if kwargs['action_input_withdenoiser'].dtype != self.dtype:
            kwargs['action_input_withdenoiser'] = kwargs['action_input_withdenoiser'].to(self.dtype)
        ...
        # print(f"DiffusionTransformer forward embeding time",time.time()-curr_time)
        curr_time = time.time()
        ...
        kwargs["act_length"] = kwargs["action_input_withdenoiser"].shape[1]

        # act_length, robot_ids
