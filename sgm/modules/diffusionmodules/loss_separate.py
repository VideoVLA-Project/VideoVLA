...
class VideoDiffusionLoss_withact_separate(StandardDiffusionLoss):
    def __init__(self, block_scale=None, block_size=None, min_snr_value=None, fixed_frames=0, vd_lw=1, act_lw=1, use_latent_action=False, latent_action_dim=50, **kwargs):
        self.fixed_frames = fixed_frames
        self.block_scale = block_scale
        self.block_size = block_size
    ...
    def __call__(self, network, denoiser, conditioner, input, batch):
        ...
        #new
        action_alphas_cumprod_sqrt, action_idx = self.sigma_sampler(action_input.shape[0], return_idx=True)
        action_alphas_cumprod_sqrt = action_alphas_cumprod_sqrt.to(input.device)
        action_idx = action_idx.to(input.device)
        ...
        additional_model_inputs["idx"] = idx
        additional_model_inputs["action_idx"] = action_idx
        additional_model_inputs["action_alphas_cumprod_sqrt"] = action_alphas_cumprod_sqrt
        ...
        action_noise_input = action_input.float() * append_dims(action_alphas_cumprod_sqrt, action_input.ndim) + action_noise * append_dims(
            (1 - action_alphas_cumprod_sqrt**2) ** 0.5, action_input.ndim
        )
        # additional_model_inputs["action_input"] = action_input
        additional_model_inputs["robot_ids"] = batch["robot_ids"] if "robot_ids" in batch.keys() else None
        # additional_model_inputs["action_noise"] = action_noise
        ...
        # [2, 13, 16, 60, 90],[2] dict_keys(['crossattn', 'concat'])  dict_keys(['idx'])
        model_output = denoiser(network, noised_input, alphas_cumprod_sqrt, action_alphas_cumprod_sqrt, cond, **additional_model_inputs)
        # model_output = model_output_dict["video_pred_noise"]
        w = append_dims(1 / (1 - alphas_cumprod_sqrt**2), input.ndim)  # v-pred

        action_model_output = additional_output_dict["action_pred_noise"]
        action_w = append_dims(1 / (1 - action_alphas_cumprod_sqrt**2), action_model_output.ndim)
