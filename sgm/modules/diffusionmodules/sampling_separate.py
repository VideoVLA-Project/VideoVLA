...
from .guiders import DynamicCFG, VanillaCFG
...
class VPSDEDPMPP2MSampler_withact(VideoDDIMSampler):
    def get_variables(self, alpha_cumprod_sqrt, next_alpha_cumprod_sqrt, previous_alpha_cumprod_sqrt=None):
        alpha_cumprod = alpha_cumprod_sqrt**2
        lamb = ((alpha_cumprod / (1 - alpha_cumprod)) ** 0.5).log()
        next_alpha_cumprod = next_alpha_cumprod_sqrt**2
    ...
    def denoise_withact(self, x, action_x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None, auxinfo_dict=None,):
        additional_model_inputs = {}
        batch = auxinfo_dict["batch"]
        ...
        if isinstance(scale, torch.Tensor) == False and scale == 1:
            ...
            # additional_model_inputs["action_noise"] = action_x
            additional_model_inputs["action_noise_input"] = action_x
            ...
            if isinstance(self.guider, DynamicCFG) or isinstance(self.guider, VanillaCFG):
                # additional_model_inputs["action_noise"] = torch.cat([action_x] * 2)
                additional_model_inputs["action_noise_input"] = torch.cat([action_x] * 2)
            else:
                # additional_model_inputs["action_noise"] = action_x
                additional_model_inputs["action_noise_input"] = action_x
            additional_output_dict = {}
            ...
            action_denoised = additional_output_dict["action_pred_noise"]
            ...
            if isinstance(self.guider, DynamicCFG):
                ...
                action_denoised = self.guider(
                    action_denoised, (1 - alpha_cumprod_sqrt**2) ** 0.5, step_index=self.num_steps - timestep, scale=scale
                )
            else:
                denoised = self.guider(denoised, (1 - alpha_cumprod_sqrt**2) ** 0.5, scale=scale)
                action_denoised = self.guider(action_denoised, (1 - alpha_cumprod_sqrt**2) ** 0.5, scale=scale)
        return denoised, action_denoised
    ...
    def sampler_step(
        ...
        # expact out put: x, old_denoised, action_x, old_action_denoised
        denoised, action_denoised = self.denoise_withact(
            x, action_x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb, auxinfo_dict=auxinfo_dict,
        )
        denoised = denoised.to(torch.float32)
        action_denoised = action_denoised.to(torch.float32)
        if idx == 1:
            return denoised, denoised, action_denoised, action_denoised
        ...
        mult_noise = append_dims((1 - next_alpha_cumprod_sqrt**2) ** 0.5 * (1 - (-2 * h).exp()) ** 0.5, x.ndim)
        action_mult = [
            append_dims(action_mult, action_x.ndim)
            for action_mult in self.get_mult(h, r, alpha_cumprod_sqrt, next_alpha_cumprod_sqrt, previous_alpha_cumprod_sqrt)
        ]
        action_mult_noise = append_dims((1 - next_alpha_cumprod_sqrt**2) ** 0.5 * (1 - (-2 * h).exp()) ** 0.5, action_x.ndim)


        x_standard = mult[0] * x - mult[1] * denoised + mult_noise * torch.randn_like(x)
        action_x_standard = action_mult[0] * action_x - action_mult[1] * action_denoised + action_mult_noise * torch.randn_like(action_x)
        if old_denoised is None or torch.sum(next_alpha_cumprod_sqrt) < 1e-14:
            # Save a network evaluation if all noise levels are 0 or on the first step
            return x_standard, denoised, action_x_standard, action_denoised
        else:
            denoised_d = mult[2] * denoised - mult[3] * old_denoised
            x_advanced = mult[0] * x - mult[1] * denoised_d + mult_noise * torch.randn_like(x)
            action_denoised_d = action_mult[2] * action_denoised - action_mult[3] * action_old_denoised
            action_x_advanced = action_mult[0] * action_x - action_mult[1] * action_denoised_d + action_mult_noise * torch.randn_like(action_x)


            x = x_advanced
            action_x = action_x_advanced

        return x, denoised, action_x, action_denoised

    def __call__(self, denoiser, x, action_x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, auxinfo_dict=None,):
        x, s_in, alpha_cumprod_sqrt, num_sigmas, cond, uc, timesteps = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
        # s_in = x.new_ones([x.shape[0]])
        ...
        for i in self.get_sigma_gen(num_sigmas):
            ...
            x, old_denoised, action_x, action_old_denoised = self.sampler_step(
                old_denoised,
                action_old_denoised,
                None if i == 0 else s_in * alpha_cumprod_sqrt[i - 1],
                s_in * alpha_cumprod_sqrt[i],
...
class VPSDEDPMPP2MSampler_withact_separate(VideoDDIMSampler):
    def __init__(self, is_vision_denoise=True, is_action_denoise=True, vision_denosie_steps=0, **kwargs):
        super().__init__(**kwargs)
        self.is_vision_denoise = is_vision_denoise
        self.vision_denosie_steps = vision_denosie_steps
        self.is_action_denoise = is_action_denoise
    ...
    def sampler_step(
        ...
        if self.is_vision_denoise:
            print("denoise X")
            x_standard = mult[0] * x - mult[1] * denoised + mult_noise * torch.randn_like(x)
        else:
            x_standard = x
        if self.is_action_denoise:
            action_x_standard = action_mult[0] * action_x - action_mult[1] * action_denoised + action_mult_noise * torch.randn_like(action_x)
        else:
            action_x_standard = action_x
...
class VPSDEDPMPP2MSampler_withact_separate_rollout(VideoDDIMSampler):
    def __init__(self, is_vision_denoise=True, is_action_denoise=True, vision_denosie_steps=0, **kwargs):
        super().__init__(**kwargs)
        self.is_vision_denoise = is_vision_denoise
        self.vision_denosie_steps = vision_denosie_steps
    ...
    def __call__(self, denoiser, x, action_x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, auxinfo_dict=None,):
        ...
        # begin vision
        setattr(self, "is_vision_denoise", True)
        setattr(self, "is_action_denoise", False)
        ...
        old_denoised = None
        action_old_denoised = None
        setattr(self, "is_vision_denoise", False)
        setattr(self, "is_action_denoise", True)
        for i in range(num_sigmas-1):
            print(f"begin action {i}")
            if self.fixed_frames > 0:
                if self.sdedit:
                    rd = torch.randn_like(prefix_frames)
                    noised_prefix_frames = alpha_cumprod_sqrt[i] * prefix_frames + rd * append_dims(
