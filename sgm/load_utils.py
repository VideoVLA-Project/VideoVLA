...
def get_checkpoint_name(checkpoints_path, iteration, release=False, use_ema=False, zero=False):
    if release:
        d = 'release'
    else:
...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None, use_ema=False,):
    ...
    checkpoint_name = get_checkpoint_name(load_path, iteration, release, use_ema)
    if mpu.get_data_parallel_rank() == 0:
