...
def add_model_config_args(parser):
    ...
    # group.add_argument("--split_weighted_oxe_data", type=bool, default=False)
...
def initialize_distributed(args):
    ...
    torch.distributed.init_process_group(
        backend=args.distributed_backend, world_size=args.world_size, rank=args.rank, init_method=init_method, timeout=timedelta(seconds=2400),
    )
    ...
    if args.deepspeed:
        ...
        deepspeed.init_distributed(
            dist_backend=args.distributed_backend, world_size=args.world_size, rank=args.rank, init_method=init_method, timeout=timedelta(seconds=2400),
        )
...
def process_config_to_args(args):
    ...
    if "action_config" in config:
        action_config = config.pop("action_config", OmegaConf.create())
        args.action_config = action_config
